...
var Game = sequelize.define('Games', {
	...
	{getterMethods: {getGame: function () {
		return [["Games"],["Game_name",this.getDataValue('Game_name')],["Conf_id",this.getDataValue('Conf_id')],
		        ["Board_id",this.getDataValue('Board_id')],["Game_goal",this.getDataValue('Game_goal')],
					...
					addGames : function(conf){
						...
						for ( var i=0 ; i<conf.Games.length ; i++){
							...
							var game1 = Game.create({
								Game_name : conf.Games[i].GAME_NAME,	
								Conf_id : conf.Global.ID,
								Board_id : conf.Games[i].Board,
								Game_goal : conf.Games[i].GameConditions.gameGoal,
